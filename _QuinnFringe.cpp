...
int QuinnFringe::getRates(double *OutRat){
  int i;
  for (i=0; i<Nant; i++){OutRat[i] = Rates[i];};
  return 0;  
};
...
int QuinnFringe::getDelays(double *OutDel){
  int i;
  for (i=0; i<Nant; i++){OutDel[i] = Delays[i];};
  return 0;  
};

int QuinnFringe::getPhases(double *OutPhs){
  int i;
  for (i=0; i<Nant; i++){OutPhs[i] = Phases[i];};
  return 0;  
};
...
QuinnFringe::QuinnFringe(int Na, int Nt, int Nc, cplx64f *ObsV, cplx64f *ModV, int *A1, int *A2, double *tA, int *dofit, double *freqs, double *wgts){
      ...
      if (BNum==j && fittable[k]){
        ...
        if (DOMODEL == 0){
          ...
          for (i=0; i<Nchan; i++){
            ...
            if(DataWeights[knu+i]<=0.0){BufferC[inu + i] = 0.0;};
          };
        } else {
          for (i=0; i<Nchan; i++){
...
// First Quadrant:
  for (l=0; l<t0; l++){
    ...
    for (k=0; k<Chi;k++){
      ...
      rmsFringe += Peak*Peak; AvgFringe += Peak;
    };
  };
// Second Quadrant:
...
// Get the SNR of the fringe (i.e., peak over RMS, but without the peak):
  ...
  rmsFringe -= AbsP*AbsP; AvgFringe -= AbsP;
...
// SNR cutoff:
  if (FringeSNR<minSNR){FringeSNR=0.0;};
...
for (i=0; i<NBas; i++){ 
  if (Weights[i] == 0.0){
    for(j=0;j<NData[i];j++){ 
      if (Tindex[i][j]>=0){fittable[Tindex[i][j]]=false;};
    };
  };
};
